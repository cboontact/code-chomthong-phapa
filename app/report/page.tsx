  ...
  return <main className="report-shell">
    ...
    <article className="report-page">
      ...
      <table className="report-table">
        <thead><tr><th className="report-col-number">ลำดับ</th><th className="report-col-batch">รุ่นที่</th><th>ชื่อรุ่น / กลุ่ม</th><th>หมายเหตุ</th><th className="report-col-date">วันที่และเวลา</th><th className="report-col-amount">จำนวนเงิน (บาท)</th></tr></thead>
        <tbody>{data.donations.length === 0 ? <tr><td colSpan={6} className="report-empty">ยังไม่มีรายการผ้าป่า</td></tr> : data.donations.map((donation, index) => <tr key={donation.id}><td className="center">{index + 1}</td><td className="center batch">{donation.batchNumber}</td><td className="name">{donation.batchName}</td><td className="note">{donation.note || "-"}</td><td className="center date">{dateTime.format(new Date(donation.receivedAt))}</td><td className="amount">{money.format(donation.amount)}</td></tr>)}</tbody>
        <tfoot><tr><td colSpan={5}>รวมทั้งสิ้น ({data.totalRecords} รายการ)</td><td className="amount">{money.format(data.totalAmount)}</td></tr></tfoot>
      </table>
